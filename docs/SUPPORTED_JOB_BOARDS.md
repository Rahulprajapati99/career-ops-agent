...
| Adzuna | API | Aggregated multi-country board via the official `api.adzuna.com/v1/api/jobs/{country}/search/{page}` API (free `app_id`/`app_key` pair — set `ADZUNA_APP_ID`/`ADZUNA_APP_KEY` in the environment, never in portals.yml). Configure with `provider: adzuna` plus per-entry `country` (default `us`), `what`, `where`, `category`, `max_days_old` (default 7), `max_pages` (default 3, cap 20). Pages are 1-based; stops early on a short page. Supplies `description` snippets and `postedAt` for free. |
...
| JSON-LD (generic) | Parser | Harvests schema.org `JobPosting` structured data from `<script type="application/ld+json">` blocks on ANY careers page that publishes it (the markup Google for Jobs consumes). Handles single objects, arrays, `@graph`, and `ItemList` wrappers; resolves relative posting URLs against the page; drops postings without their own URL (dedup-key safety). Not auto-detected — configure explicitly with `provider: jsonld` + `careers_url:` per page. |
